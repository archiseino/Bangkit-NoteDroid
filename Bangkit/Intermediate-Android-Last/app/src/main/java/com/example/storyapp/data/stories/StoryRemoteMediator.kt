...
class StoryRemoteMediator (private val apiService: StoryService, private val appDatabase : AppDatabase) : RemoteMediator<Int, ListStoryItem>() {
    ...
    override suspend fun load(
        ...
        try {
            val response = apiService.getStories(page, state.config.pageSize).listStory
            ...
            appDatabase.withTransaction {
                ...
                appDatabase.storyDao().insertStory(response)
            }
    ...
    private suspend fun getRemoteKeyForLastItem(state: PagingState<Int, ListStoryItem>): RemoteKeys? {
        return state.pages.lastOrNull { it.data.isNotEmpty() }?.data?.lastOrNull()?.let { data ->
            appDatabase.remoteKeyDao().getRemoteKeysId(data.id)
        }
    }
    ...
    private suspend fun getRemoteKeyClosestToCurrentPosition(state: PagingState<Int, ListStoryItem>): RemoteKeys? {
        return state.anchorPosition?.let { position ->
            state.closestItemToPosition(position)?.id?.let { id ->
                appDatabase.remoteKeyDao().getRemoteKeysId(id)
            }
        }
    }
